...
export function getGlassSurfaceCount() {
  return surfaces.size + (navBlurEntry ? 1 : 0);
}

export function setGlassSceneApi(apiRef) {
  sceneApiRef = apiRef;
  if (surfaces.size > 0 || navBlurEntry) attachRenderHook();
}

function syncAllSurfaces() {
  if (!pageVisible || (surfaces.size === 0 && !navBlurEntry)) return;
...
function attachRenderHook() {
  ...
  if (surfaces.size === 0 && !navBlurEntry) return;
...
export function registerGlassSurface(shellRef, canvasRef) {
  ...
  return () => {
    ...
    if (surfaces.size === 0 && !navBlurEntry) {
      detachRenderHook();
      removeGlobalListenersIfIdle();
    }
  };
